...
function generateDownloadScript(zoom = 1) {
    ...
    console.log('# Download all tiles (adjust ranges based on detectTiles output)');
    console.log('for x in {0..10}; do');
    console.log('  for y in {0..10}; do');
    console.log(`    curl -o tile_${zoom}-\${x}-\${y}.png "${baseUrl}/tile_${zoom}-\${x}-\${y}.png" 2>/dev/null || true`);
    console.log('  done');
    console.log('done');
}
...
if (result && result.tiles.length > 0) {
    ...
    console.log('2. Run generateDownloadScript(zoom) with your desired zoom level');
